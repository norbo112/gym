...
function makeAdatOsszesito(obj, target, hiba) {
    ...
    for (var i = 0; i < obj.result_sajat.length; i++) {
        ...
        sulytomb.push("diag" + i + "_" + obj.result_sajat[i].osszsuly);
    ...
    if (obj.result_partner) {
        ...
        for (var i = 0; i < obj.result_partner.length; i++, index_tomb++) {
            ...
            sulytomb.push("diag2" + index_tomb + "_" + obj.result_partner[i].osszsuly);
    ...
    for (var i = 0; i < sulytomb.length; i++) {
        var adat = sulytomb[i].split("_");
        var s1 = (adat[1] / allinosszuly) * 100;
        $("#" + adat[0]).css("width", Math.floor(s1) + "%");
        $("#" + adat[0]).html(adat[1]);
    }
}
